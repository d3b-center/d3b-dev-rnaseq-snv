...
id: sentieon-dnascope
label: Sentieon DNAscope
doc: |-
  Variant Calling using DNAscope
...
- position: 2
  shellQuote: false
  valueFrom: >-
    $(inputs.output_basename).dnascope.vcf.gz
...
inputs:
  ...
  output_basename: { type: 'string?', doc: "basename of output vcf file. suffix .dnascope.vcf.gz will be added to it",
...
outputs:
  rna_snv_vcf: { type: File, outputBinding: { glob: "$(inputs.output_basename).dnascope.vcf.gz"}, secondaryFiles: [".tbi"] }
